...
describe("Slack -> Chatwoot", () => {
  it("creates contact, conversation and message for a top-level post", async () => {
    ...
    expect(actions?.elements?.[0]).toMatchObject({ action_id: "chatwoot_bridge_resolve", value: "resolve:1700000000.000100", text: { text: "Resolve" } });
    // ...and the welcome ts is remembered so the button can be re-labelled later.
    expect((await bridge.db.select().from(threads))[0]!.welcomeMessageTs).toBeTruthy();
