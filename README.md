...
- The welcome message carries a **Resolve** button (label configurable, blank hides it) that the asker or any linked agent can click. Anyone else gets a private note explaining why not. The button needs Interactivity enabled on the bridge's Slack app, at the same request URL as its events.
